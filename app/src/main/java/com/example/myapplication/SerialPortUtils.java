...
public class SerialPortUtils {
    ...
    public void openSerialPort(String path, String suPath){
        ...
        try {
            ...
            System.out.println(TAG + " openSerialPort: 打开串口异常：" + e.toString());
        }
    ...
    /**
     * 用于串口发送数据
     * 不需要计算出 crc 值，该方法里会自动计算 crc 值
     * @param data 需要发送的数据
     */
    public boolean sendBytesSerialPort(byte[] data){
        boolean result = false;
        byte[] bytes = data;
//        byte[] bytes = DataOperationUtils.addCycBytes(data, CRC_VERIFY_LENGTH); // TODO need find alternative
